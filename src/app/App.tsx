...
function App(): JSX.Element {
    ...
    const handleChangeVideoDevice = useCallback(async (deviceId: string | undefined): Promise<void> => {
        await mediaService?.changeVideoDevice(deviceId);
    }, [mediaService]);

    const handleChangeAudioDevice = useCallback(async (deviceId: string | undefined): Promise<void> => {
    ...
    const handleDeviceChange = useCallback(({videoTrack, audioTrack}: IMediaTracks) => {
        setCurrentVideoDevice(videoTrack);
        setCurrentAudioDevice(audioTrack);
    }, []);

    useEffect(() => {
        const mediaService = new MediaService(handlePlay, handlePause, handleDeviceChange);
